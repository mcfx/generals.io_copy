import random,time,eventlet,threading,requests
...
def chkconn(grid_type,n,m):
	fa=[i for i in xrange(n*m)]
	sz=[1 for i in xrange(n*m)]
	def find(x):
		if x==fa[x]:return x
		fa[x]=find(fa[x])
		return fa[x]
	...
	for i in xrange(n):
		for j in xrange(m):
			if grid_type[i][j]!=1:
				if i+1<n and grid_type[i+1][j]!=1:merge(i*m+j,i*m+j+m)
				if j+1<m and grid_type[i][j+1]!=1:merge(i*m+j,i*m+j+1)
			else:
				cnt+=1
	for i in xrange(n*m):
		if sz[i]>(n*m-cnt)*0.9:
...
class Game:
	def __init__(self,game_conf,update,player_ids,chat_message,gid,md5):
		self.md5=md5
	...
	def getcustommap(self,title):
		try:
			r=requests.get('http://generals.io/api/map',params={'title':title.encode('utf-8')}).json()
			n=r['height']
			m=r['width']
			t=r['map'].split(',')
			self.owner=[[0 for j in xrange(m)] for i in xrange(n)]
			self.army_cnt=[[0 for j in xrange(m)] for i in xrange(n)]
			self.grid_type=[[0 for j in xrange(m)] for i in xrange(n)]
			for i in xrange(n):
				for j in xrange(m):
					x=t[i*m+j].strip(' ')
					if x=='m':
			...
			self.n=n
			self.m=m
			self.st=[[False for j in xrange(m)] for i in xrange(n)]
	...
	def genmap(self):
		...
		while True:
			grid_type=[[0 for j in xrange(m)] for i in xrange(n)]
			for i in xrange(n):
				for j in xrange(m):
					tmp=random.random()
	...
	def sel_generals(self):
		...
		for T in xrange(100):
			...
			if self.rpcnt>len(ge):
				for i in xrange(min(self.rpcnt-len(ge),len(sp))):
					ge.append(sp[i])
			if self.rpcnt>len(ge):
				for i in xrange(self.pcnt-len(ge)):
					ge.append((-1,-1))
			random.shuffle(ge)
			tv=233333
			for i in xrange(self.rpcnt):
				for j in xrange(i):
					tv=min(tv,abs(ge[i][0]-ge[j][0])+abs(ge[i][1]-ge[j][1]))
			if tv>ge_val:
				ge_r=ge
				ge_val=tv
		for i in xrange(self.n):
	...
	def sendmap(self,stat):
		...
		pl_v=[[0,0] for i in xrange(self.pcnt)]
		for i in xrange(self.n):
			for j in xrange(self.m):
				if self.owner[i][j]:
		...
		for i in xrange(self.pcnt):
			...
			if self.pstat[i]==left_game:
				...
				cl='afk'
			if self.team[i]:
				leaderboard.append({'team':self.team[i],'uid':self.names[i],'army':pl_v[i][0],'land':pl_v[i][1],'class_':cl,'id':i+1})
		for p in xrange(self.pcnt):
			if self.watching[p]:
				rt=[[0 for j in xrange(self.m)] for i in xrange(self.n)]
				rc=[[0 for j in xrange(self.m)] for i in xrange(self.n)]
				for i in xrange(self.n):
					for j in xrange(self.m):
						if self.team[p]==0 or self.spec[p]:
							rt[i][j]=200
						else:
							rt[i][j]=202
							for d in xrange(9):
		...
		for p in xrange(self.pcnt):
			if self.pstat[p]:
				...
				if self.pstat[p]==left_game-1:
					self.kill(0,p+1)
		tmp=range(self.pcnt)
		if self.turn%2==1:tmp=list(reversed(tmp))
		self.lock.acquire()
		for p in tmp:
			while len(self.pmove[p]):
				mv=self.pmove[p].pop(0)
				if not self.chkmove(mv[0],mv[1],mv[2],mv[3],p):continue
