...
def enter_room(game_id):
	game_id=unicode(game_id)
	if len(game_id)==0 or len(game_id)>15:
		return redirect('/games/'+(''.join([unichr(random.randint(0,25)+ord('a'))for i in range(4)])))
	return app.send_static_file('index.html')

@app.errorhandler(404)
def enter_random_room(e):
	return redirect('/games/'+(''.join([unichr(random.randint(0,25)+ord('a'))for i in range(4)])))
...
@socketio.on('attack')
def on_attack(data):
	if game_uid.has_key(request.sid):
...
def leave_game_room(sid,gid):
	for i in xrange(len(gr_players[gid])):
...
def getval(gid):
	if not gr_val.has_key(gid):
		gr_val[gid]=md5(gid.encode('utf-8')+str(time.time()))
	return gr_val[gid]

@socketio.on('join_game_room')
def on_join_game_room(data):
	data['room']=unicode(data['room'])
	ioroom=getval(data['room'])
	data['nickname']=unicode(data['nickname'])
	if not gr_id.has_key(request.sid):
...
def on_change_nickname(data):
	data['nickname']=unicode(data['nickname'])
	if gr_id.has_key(request.sid) and len(data['nickname'])<15 and len(data['nickname'])>0:
...
def on_change_team(data):
	...
	if gr_id.has_key(request.sid) and data['team']>=0 and data['team']<=max_teams:
		gid=gr_id[request.sid]
		ioroom=getval(gid)
		for i in gr_players[gid]:
			if i[0]==request.sid:
...
def chk_ready(gid,ioroom):
	...
	for i in gr_players[gid]:
		if i[3]:rcnt+=1
	#print rcnt,get_req_ready(len(gr_players[gid]))
...
def on_change_game_conf(data):
	...
	tmp['custom_map']=unicode(data['custom_map'])
	if gr_id.has_key(request.sid) and len(tmp['custom_map'])>=0 and len(tmp['custom_map'])<100:
		gid=gr_id[request.sid]
		ioroom=getval(gid)
		mess_q=[]
...
def chk_leave():
	if gr_id.has_key(request.sid):
		...
		chk_ready(gid,ioroom)
	elif game_uid.has_key(request.sid):
...
def start_game(gid):
	...
	game=Game(grc,emit_update,player_sids,chat_message,gid,md5)
...
def on_send_message(data):
	...
	elif gr_id.has_key(request.sid):
		gid=gr_id[request.sid]
		ioroom=getval(gid)
